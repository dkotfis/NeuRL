...
public class LevelGenerator
{
    ...
    public static Level createLevel(int width, int height, long seed, int difficulty, int type)
    {
    	if (GlueMario.glue_running) {
    		return GlueMario.param.getGenerator().createLevel(seed, difficulty, type);
    	}
    	else {
    ...
    protected int buildCannons(int xo, int maxLength)
    {
    	if (GlueMario.glue_running)
    		return 0;
    ...
    protected void decorate(int x0, int x1, int floor)
    {
        if (floor < 1) return;
